...
def FormatAndSplit(df, cut_date):
    """ Preprocess data into CustomerNameIdx, Ticker, Rating dataframe.
...
def GetClosestCustomers(ticker_id, train, rank=10, max_customers=20):
    
    M, N = GetLatentSpace(train)
    
    centroidN = N[ticker_id]
    
    # Get closest CustomerNameIdxs
    d = M.dot(centroidN)
    idx = d.argsort()[-max_customers:][::-1]
    return idx
